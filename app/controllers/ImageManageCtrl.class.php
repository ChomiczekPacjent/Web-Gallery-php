...
class ImageManageCtrl {
    ...
    public function action_remove_all_images() {
        ...
        if (!$this->form->propertyId) {
            App::getMessages()->addMessage(new Message('Brak ID.', Message::ERROR));
            $this->generateView();
            return;
        }
        ...
        if (!$property) {
            App::getMessages()->addMessage(new Message('Własność nie istnieje.', Message::ERROR));
            $this->generateView();
            return;
        }
        ...
        App::getSmarty()->assign('page_title', "Edytuj");
        $this->generateView();
    }
